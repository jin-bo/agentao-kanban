...
**当前版本:v0.1.9-dev**(已发布最新版 v0.1.8)。完整变更见 [CHANGELOG.md](CHANGELOG.md)。
...
一张卡的「结果」其实分散在三个地方,不要被 `kanban worktree list` 的
`No active worktree directories` 误导成"结果丢了":

- **代码改动**:`kanban/<card-id>` 分支(detach 后目录消失,分支保留)
- **gitignored 交付物**:`workspace/raw/<card-id>/artifacts-<ts>/`(detach
  前抢救的快照)
- **执行记录**:`outputs.last.summary`、`workspace/raw/<card>/<role>-<ts>.md`
  raw transcript、`events.log` 事件流

`kanban result <card-id>` 把这些统一展示出来,并给出下一步命令:

```bash
uv run kanban result <card-id>           # 状态 + summary + branch + artifacts + transcripts + 下一步
uv run kanban result <card-id> --json    # 同上的机器可读版本
uv run kanban worktree diff <card-id>    # 看分支上的具体改动
uv run kanban traces <card-id> --latest  # 看最近一份 raw transcript
```

`kanban show <card-id>` 也会在 YAML/JSON 顶层加一个 `result:` 区块,把
worktree 状态(`active`/`detached`/`missing`)、artifacts、transcripts、
建议的下一步命令都摊开。
