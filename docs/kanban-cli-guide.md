...
第一站永远是 `kanban result`——一条命令把状态、worker summary、worktree 状态、
artifact 快照路径、retained transcripts 与建议的下一步都汇到一个视图:

```bash
uv run kanban result <card_id>          # 总览
uv run kanban result <card_id> --json   # 喂下游脚本
```

需要更细的视角再用:

```bash
uv run kanban show <card_id>            # 完整卡定义(YAML/JSON,含 result: 块)
uv run kanban events <card_id>          # 结构化事件流
uv run kanban worktree diff <card_id>   # 分支上具体改了什么(active/detached 都行)
uv run kanban traces <card_id> --latest # agent 原始 transcript
```

> 看到 `kanban worktree list` 不显示这张卡**不等于结果丢了**:卡进入 DONE/BLOCKED 时
> worktree 目录会 detach,但分支(`kanban/<card-id>`)和 artifacts 都还在。`kanban result`
> 会把这一切翻译成可读的状态串。
...
```bash
uv run kanban result <card_id>          # blocked_reason + 已有产物 + 建议下一步
uv run kanban events <card_id>          # 完整事件流(含 BLOCKED 触发那一条)
```
...
```bash
uv run kanban result <card_id>          # 状态 + 阻塞原因(若有) + 当前产物
uv run kanban claims <card_id>          # 是否有 live claim
uv run kanban events <card_id>          # 最近事件流
uv run kanban doctor                    # 板/环境是否有结构性问题
```
...
# 2) 可写入(loopback)。暴露建卡 + move/requeue/block/unblock 五项写入面。
...
`--enable-writes` 是单一开关。打开后暴露以下写入面,其余 surface 保持只读:

| 路由 | body | 等价 CLI |
|---|---|---|
| `POST /api/cards` | `{title, goal, priority, acceptance_criteria, depends_on}` | `kanban card add` |
| `POST /api/cards/{id}/move` | `{status}` | `kanban move <id> <status>` |
| `POST /api/cards/{id}/requeue` | `{target, note?}`(`target` ∈ `inbox`/`ready`) | `kanban requeue <id> --to ...` |
| `POST /api/cards/{id}/block` | `{reason}` | `kanban block <id> "reason"` |
| `POST /api/cards/{id}/unblock` | `{target}` | `kanban unblock <id> --to ...` |

`move`/`requeue`/`block`/`unblock` 调的是 `kanban.operations.transition_*`
共享函数,语义与 CLI / MCP 完全一致(单次卡写入落盘即 commit point;落到
`DONE` 时推进 inbox 依赖、落到 `DONE`/`BLOCKED` 时 detach worktree 都是
commit 之后的 best-effort,失败只作为响应里的 `warnings` 返回、不回滚)。
成功响应是 `{ "card": <card>, "warnings": [...] }`;web 不暴露 `--force`、批量
操作、daemon 启停,也没有 worktree merge/prune/delete/checkout 路由。

绑定到非 loopback 主机(`0.0.0.0`、`192.168.x.x` 等)且未带
`--allow-remote-writes` 时,server 启动直接拒绝,避免误开放写入面到内网。

### 9.2 写入面与 `.daemon.lock` / live claim 的关系
...
- `POST /api/cards` **不**与 `.daemon.lock` 串行化,daemon 跑着也能加卡。
  ...
  注释
- 既有卡的四项写入(`move`/`requeue`/`block`/`unblock`)有更严的门:
  - 未带 `--enable-writes` → `403`(注意与 `POST /api/cards` 关写时的 `405`
    区分)
  - `.daemon.lock` 活着(daemon 正在跑)→ `409 daemon_active`
  - 目标卡有 live execution claim → `409 live_claim`
  - 没有 `--force` 逃生口
  失败响应是稳定的 envelope:`{ "error": "...", "message": "...", "retryable": bool }`,
  不带 traceback。preflight(权限 / 卡是否存在 / 状态合法性 / `block` 非空
  reason)在任何写入之前完成,被拒的请求不改动卡文件与 `events.log`
- web 进程**不会**清理 stale lock — 那只在 `kanban daemon` 启动时由 daemon
  自己接管(检查 pid 是否还活着,不活就清掉再起)

实际工作流:daemon 跑着的时候,web 上的"加卡"按钮**仍可用**(新卡 race-free);
但 `move`/`block` 等改既有卡的按钮会拒(`409`),先停 daemon 再操作。卡详情
弹窗里的 Actions 区块只在 `--enable-writes` 时出现,目标卡有 live claim 时控件
置灰。需要彻底禁掉 web 写入就把 `--enable-writes` 关掉 — 这是单开关。
...
如果你看到 worker 报告写了文件但卡完成后找不到,**第一站是
`uv run kanban result <card-id>`**——它会列出最新一份 artifact 快照路径(以及
worktree 分支、retained transcripts 和建议的下一步),不用再手扒
...
| **一卡总览(状态+产物+下一步)** | `uv run kanban result <card_id>` |
| 看事件 | `uv run kanban events <card_id>` |
| 看分支上的改动 | `uv run kanban worktree diff <card_id>` |
...
如果团队里有人只需要先上手最核心能力,先学这 11 条就够了:
...
uv run kanban move <card_id> ready
uv run kanban daemon
uv run kanban result <card_id>            # 一卡总览(状态+产物+下一步)
uv run kanban events <card_id>
